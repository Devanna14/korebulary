...
export default function Flashcard({ card, alEscuchar }) {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full duration-700 preserve-3d shadow-2xl rounded-3xl ${isFlipped ? "rotate-y-180" : ""}`}
      >
        ...
        <div className="absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-center bg-gradient-to-br from-indigo-600 via-blue-600 to-blue-500 text-white rounded-3xl p-8 shadow-inner">
          <button
            onClick={(e) => {
              e.stopPropagation(); // Evita que la tarjeta se voltee al dar click al sonido
              alEscuchar();
            }}
            className="p-2 mb-4 bg-blue-100 rounded-full hover:bg-blue-200 transition-colors"
          >
            🔊
          </button>
